...
const char *EnvMapShaderSource = R"(
	...
	void main()
	{
		...
		for(int i=0;i < NumSamples;i++)
		{
			...
			//
			vec3 NonMetallicContrib = CalculateNonMetallicContrib(vec3(0.0),Fresnel,CameraDotNormal,SurfaceRoughness,Reflectiveness);
			vec3 MetallicContrib = CalculateMetallicContrib(Albedo,CameraDotNormal,SurfaceRoughness);
			//
			/*
			 * Adding it all up
			 */
