...
#include <lScene/l3DScene/lModScene/li3DModElement.h>
#include "lr3DCamera.h"
...
#include <lScene/l3DScene/lModScene/li3DModMesh.h>
...
class lr3DSkeletalMesh : public lr3DMesh, public li3DModSkeletalMesh
{
protected:
...
public:
	//
	virtual void SetCharacterName(const std::string &character_name) override
	{
	...
	virtual void Accept(li3DModMeshVisitor &visitor) override
	{
		visitor.VisitSkeletalMesh(*this);
	}
	//
	virtual const std::string &GetCharacterName() const override
	{
	...
	{}
	//
	virtual ~lr3DSkeletalMesh() override
	{}
	/*
	 * End of class
	 */
...
protected:
	...
	private:
		lr3DScene &Scene;
		//
		void PopulateMesh(lr3DMesh &mesh,const lmVector3D &position,const lmQuaternion &orientation,const lmVector3D &scale,const liMaterialLibrary &mtl_lib)
		{
			mesh.SetPosition(position);
			mesh.SetOrientation(orientation);
			mesh.SetScale(scale);
			//
			mesh.SetMaterialLibrary(mtl_lib);
		}
		//
	public:
		//
		virtual li3DModElement *CreateStaticMesh(const lmVector3D &position,const lmQuaternion &orientation,const lmVector3D &scale,const liMaterialLibrary &mtl_lib,const std::string &model_name) override
		{
			...
			#endif
			//
			PopulateMesh(*StaticMesh,position,orientation,scale,mtl_lib);
			//
		...
		//
		virtual li3DModElement *CreateSkeletalMesh(const lmVector3D &position,const lmQuaternion &orientation,const lmVector3D &scale,const liMaterialLibrary &mtl_lib,const std::string &character_name) override
		{
			#ifdef LR3D_SCENE_USE_OBJECT_POOL
			lr3DSkeletalMesh *SkeletalMesh;
			lr3DElementPool *ElementPool = nullptr;
			if(Scene.ElementPools.size() > 0)
			{
				ElementPool = Scene.ElementPools.back();
			...
			//
			if(ElementPool == nullptr || (ElementPool != nullptr && !ElementPool->CanAllocate<lr3DSkeletalMesh>()))
			{
				ElementPool = Scene.AllocateElementPool();
			}
			ElementPool->AllocateElement(SkeletalMesh);
			#else
			lr3DSkeletalMesh *SkeletalMesh = new lr3DSkeletalMesh();
			//
			Scene.Elements.push_back(SkeletalMesh);
			#endif
			//
			PopulateMesh(*SkeletalMesh,position,orientation,scale,mtl_lib);
			//
			SkeletalMesh->SetCharacterName(character_name);
			//
			return SkeletalMesh;
		}
		//
